...
export default function LeaderboardScreen({
  ...
  const [apiBoard, setApiBoard] = useState<LeaderboardEntry[] | null>(null);
  const [loading, setLoading] = useState(true);

  // Загружаем лидерборд из API
  useEffect(() => {
    async function load() {
      setLoading(true);
      const data = await fetchLeaderboard(tab, currentFid);
      if (data.length > 0) {
        setApiBoard(data);
      } else {
        setApiBoard(null); // Используем демо-данные
      }
      setLoading(false);
    }
    load();
  ...
  if (apiBoard && apiBoard.length > 0) {
    ...
    // Демо-данные (fallback для нового приложения без пользователей)
  ...
  return (
    <div className="px-5 pb-24 animate-in">
      ...
      <p className="text-gray-500 text-sm mb-5">
        {apiBoard ? "Top learners this week" : "Demo leaderboard — connect via Farcaster for real data"}
      </p>
      ...
      {loading && (
        <div className="text-center py-8">
          <div className="w-8 h-8 border-2 border-base-blue border-t-transparent rounded-full animate-spin mx-auto mb-2" />
